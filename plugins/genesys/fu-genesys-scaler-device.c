...
fu_genesys_scaler_device_setup(FuDevice *device, GError **error)
{
	...
	/* the flash ID can only be read in ISP mode, which blanks the panel, so it has to come
	 * from a quirk */
...
fu_genesys_scaler_device_new(FuDevice *proxy)
{
	...
	device = g_object_new(FU_TYPE_GENESYS_SCALER_DEVICE, "proxy", proxy, NULL);
	return device;
}
