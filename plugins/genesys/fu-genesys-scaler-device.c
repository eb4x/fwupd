...
fu_genesys_scaler_device_attach(FuDevice *device, FuProgress *progress, GError **error)
{
	...
	/* leaving ISP mode resets the scaler, and on some panels the hub re-enumerates too */
...
fu_genesys_scaler_device_probe(FuDevice *device, GError **error)
{
	...
	if (self->has_public_key) {
		...
		if (proxy == NULL)
			return FALSE;
		/* limit the match to one product with the hub IDs, as the public key does for level 0 */
